...
class LlamaDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        encoding = [self.tokenizer.encode(
            s, bos=True, eos=self.eos) for s in sents]
        max_length_in_batch = max([len(sentence) for sentence in encoding])
        encoding_padded = [sentence + [self.tokenizer.pad_id] *
                           (max_length_in_batch - len(sentence)) for sentence in encoding]
...
def train_lora(args):
    ...
    model.llama = apply_lora(
        model.llama, rank=args.lora_rank, alpha=args.lora_alpha)
    ...
    print(
        f"Training {len(trainable_params)} parameter groups (LoRA + classifier)")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        print(
            f"LoRA epoch {epoch}: train loss :: {train_loss :.3f}, train acc :: {train_acc :.3f}, dev acc :: {dev_acc :.3f}")
...
def generate_sentence(args, prefix, outfile, max_new_tokens=75, temperature=0.0):
    with torch.no_grad():
        ...
        ctx = torch.amp.autocast(
            device_type="cuda", dtype=torch.float32) if args.use_gpu else nullcontext()
        ...
        x = (torch.tensor(start_ids, dtype=torch.long,
             device=device)[None, ...])
...
def test_with_prompting(args):
    assert args.dev_out.endswith(
        "dev-prompting-output.txt"), 'For saving prompting results, please set the dev_out argument as "<dataset>-dev-prompting-output.txt"'
    assert args.test_out.endswith(
        "test-prompting-output.txt"), 'For saving prompting results, please set the test_out argument as "<dataset>-test-prompting-output.txt"'
    ...
    with torch.no_grad():
        ...
        if len(label_names) == 2:
            ...
            label_name_str = ", ".join(
                label_names[:-1]) + ", or " + label_names[-1]
        ...
        test_data = create_data(args.test, tokenizer,
                                'test', eos=False, prompt_suffix=prompt_suffix)
        ...
        write_predictions_to_file(
            "dev", args.dev_out, dev_acc, dev_pred, dev_sents)
        write_predictions_to_file(
...
def get_args():
    ...
    parser.add_argument("--label-names", type=str,
                        default="data/cfimdb-label-mapping.json")
    parser.add_argument("--pretrained-model-path",
                        type=str, default="stories42M.pt")
    ...
    parser.add_argument("--generated_sentence_low_temp_out",
                        type=str, default="generated-sentence-temp-0.txt")
    parser.add_argument("--generated_sentence_high_temp_out",
                        type=str, default="generated-sentence-temp-1.txt")
    parser.add_argument("--dev_out", type=str,
                        default="cfimdb-dev-prompting-output.txt")
    parser.add_argument("--test_out", type=str,
                        default="cfimdb-test-prompting-output.txt")
    parser.add_argument("--lora_rank", type=int,
                        default=4, help="LoRA rank (r)")
    parser.add_argument("--lora_alpha", type=float,
                        default=1.0, help="LoRA alpha scaling factor")
...
if __name__ == "__main__":
    ...
    if args.option == "generate":
        ...
        generate_sentence(args, prefix, args.generated_sentence_low_temp_out,
                          max_new_tokens=75, temperature=0.0)
        generate_sentence(args, prefix, args.generated_sentence_high_temp_out,
                          max_new_tokens=75, temperature=1.0)
